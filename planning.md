...
### Evaluation Metrics:
**Primary metric: Macro F1** — averages F1 equally across all three labels (`analysis`, `reaction`, `inquiry`). Chosen because class imbalance is likely (r/running skews toward `reaction` and `inquiry`), and accuracy alone would reward a model that ignores rare classes like `analysis`.
...
This classifier is considered genuinely useful at a MacroF1 of 0.70 or above across all three labels on the held-out test set. 

At 0.70, the model reliably distinguished the core label pairs(`analysis` vs. `reaction`, `reaction` vs. `inquiry`) well enough to assist a human moderator or researcher - not to replace human judgment, but to triage and surface patterns at scale. 
...
- Attempted automated collection via PRAW and Arctic Shift API — both returned predominantly AutoModerator threads and [removed] posts
- Collected 200+ posts manually from r/running Top (This Month) feed
- Posts were labeled at collection time by the human annotator directly against the label definitions and tiebreaker rules in this document
- No AI pre-labeling was used — all labels reflect direct human judgment
- Final human-assigned label is the ground truth

**Tracking table format:**

| post_id | title | body | score | ai_label | label | ai_assisted |
|---|---|---|---|---|---|---|
| Reddit post ID | Post title | Post body text | Reddit upvotes | N/A not used | Final human-reviewed label | `False` |

**Disclosure note:** Due to scraper failures, the planned Claude pre-labeling pipeline was not executed. Every label was assigned directly by the human annotator. The ai_assisted column is False for all rows in this dataset. Claude was used for label stress-testing and failure analysis but not for individual post annotation.
